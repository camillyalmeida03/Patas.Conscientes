...
// Controller responsável por receber requisições HTTP e chamar o service da tabela racaspet para executar o CRUD.

const model = require("../models/racaspetsServices");

const racaspetController = {
    ...
    Put: async (request, response) => {
        try {
            ...
            const data = await model.Put(id, raca, fk_idespeciepet); // chama service sem passar response
            response.status(200).json(data);
        } catch (error) {
            console.error("Erro ao conectar ao banco de dados:", error.message);
            response.status(500).json({ message: "Falha ao executar a ação!" });
