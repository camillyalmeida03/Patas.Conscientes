// Controller responsável por receber requisições HTTP e chamar o service da tabela sexo para executar o CRUD.
...
const sexoController = {
    ...
    Post: async (request, response) => {
        try {
            ...
            const data = await model.Post(sexo); // chama service sem passar response
    ...
    Put: async (request, response) => {
        try {
            ...
            const data = await model.Put(id, sexo); // chama service sem passar response
            response.status(200).json(data);
        } catch (error) {
            console.error("Erro ao conectar ao banco de dados:", error.message);
            response.status(500).json({ message: "Falha ao executar a ação!" });
